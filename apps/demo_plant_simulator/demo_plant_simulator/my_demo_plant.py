...
class MyDemoPlant(Plant):
    def __init__(self, evaluate: bool, path: str):
        super().__init__(evaluate, sampler_buffer_size=57600, sample_group_size=1, stats_size=int(2 * 3600 / (Plant.TIME_INCREMENT * 6)), stats_period=1800)

        # Create and link material handlers
        source_a = SimulatedMine("Great Mine", max_tph=4000, availability=0.9, q_min=0.20, q_exp=0.23, q_max=0.30, plant=self)
        source_b = SimulatedMine("Huge Mine", max_tph=11300, availability=0.99, q_min=0.28, q_exp=0.35, q_max=0.40, plant=self)
        source_c = SimulatedMine("Some Mine", max_tph=7000, availability=0.97, q_min=0.25, q_exp=0.31, q_max=0.38, plant=self)
        ...
        duplicator_a = MaterialDuplicator("Duplicator A", src=MaterialBuffer("Mux Transport A", src=mux_a, steps=1, plant=self), count=2, plant=self)
        blend_a_chevron_50 = BlendingSystem(
            "Blending Strategy B", src=(duplicator_a, 1), strategy="Chevron", strategy_params={"layers": 50, "by": "mass"}, plant=self
        )
        ...
        self.material_outs = [out_a_chevron_50, out_a_pile, out_b]
