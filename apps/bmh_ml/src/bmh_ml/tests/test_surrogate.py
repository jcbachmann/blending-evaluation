...
def surrogate_files(tmp_path, monkeypatch):
    ...
    make_model(FEATURES).save(get_model_file(MODEL_F1_NAME))
    make_model(DEPOSITION_LENGTH).save(get_model_file(MODEL_F2_NAME))
...
def test_predict_returns_both_objectives_for_every_row():
    ...
    predictions = Surrogate.load().predict(x)
...
def test_problem_fixes_the_material_and_bounds_the_deposition():
    ...
    problem = HomogenizationProblemMl(surrogate=Surrogate.load(), material_variables=material)
...
def test_models_written_by_the_trainer_can_be_used_by_the_surrogate(tmp_path, monkeypatch):
    ...
    train_lstm_model.main(argparse.Namespace(verbose=False, epochs=1))

    predictions = Surrogate.load().predict(np.full((2, FEATURES), 20.0))
