...
from blending_simulator.material_deposition import MaterialMeta, DepositionMeta
...
class Benchmark:
    ...
    def read_materials(self, path: str) -> Dict[str, MaterialMeta]:
        ...
        self.materials = Benchmark.create_instance_for_each_managed_dir(path, MaterialMeta)
    ...
    def read_depositions(self, path: str) -> Dict[str, DepositionMeta]:
        ...
        self.depositions = Benchmark.create_instance_for_each_managed_dir(path, DepositionMeta)
    ...
    def read_simulators(self, path: str) -> Dict[str, SimulatorMeta]:
        ...
        self.simulators = Benchmark.create_instance_for_each_managed_dir(path, SimulatorMeta)
    ...
    def read_references(path: str) -> Dict[str, ReferenceMeta]:
        logging.debug('Reading references')
        references = Benchmark.create_instance_for_each_managed_dir(path, ReferenceMeta)
        logging.info(f'{len(references)} references read')
        return references

    def read_base(self, path: str) -> None:
        self.read_materials(os.path.join(path, Benchmark.MATERIAL_DIR))
        self.read_depositions(os.path.join(path, Benchmark.DEPOSITION_DIR))
        self.read_simulators(os.path.join(path, Benchmark.SIMULATOR_DIR))
    ...
    def list_managed_dirs(path: str) -> List[str]:
        ...
        return [
            ...
            if
            os.path.isdir(os.path.join(path, entry)) and os.path.isfile(os.path.join(path, entry, Benchmark.META_JSON))
        ]
    ...
    def create_instance_for_each_managed_dir(path: str, instance_type: type) -> dict:
        ...
        entries = {}
        for entry in Benchmark.list_managed_dirs(path):
            ...
            meta = json.load(open(os.path.join(entry_path, Benchmark.META_JSON)))
