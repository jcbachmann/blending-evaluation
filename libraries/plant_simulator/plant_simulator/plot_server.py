...
def create_live_graphs(source, palette, columns):
    ...
    for col in columns:
        if " quality" in col:
            p_live_q.line(x="time", y=col, source=source, color=palette[i], legend=col.replace(" quality", ""))
            i += 1
...
def create_stats_graphs(source, palette, p_reference, columns):
    ...
    for col in columns:
        if " quality" in col:
            ...
            p_stats_q.line(x="time", y=f"{col} average", source=source, color=palette[i], legend=col.replace(" quality", ""), line_width=3)
            i += 1
...
def get_stats_columns(columns):
    ...
    for col in columns:
        stats_columns.extend(
            ...
            if " tph" in col or " quality" in col
            else [col]
        )
