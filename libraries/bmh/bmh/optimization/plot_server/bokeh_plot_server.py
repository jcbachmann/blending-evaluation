...
class BokehPlotServer(PlotServer):
    ...
    def make_document(self, doc: Document) -> None:  # noqa: C901
        ...
        path_fig.line(x="timestamp", y="x", legend_label="Reference", source=reference_path_source, color=palette[4], line_dash="4 4", alpha=0.5)
        path_fig.ray(x="t_start", y=0, color="black", length=0, angle=90, angle_units="deg", alpha=0.5, source=progress_source)
        ...
        for i, p in enumerate(parameter_labels):
            material_input_fig.line(
                x="timestamp",
                y=p,
                legend_label=f"Parameter {p}",
                source=material_input_source,
                color=palette[i],
            )
        ...
        for i, p in enumerate(parameter_labels):
            ...
            material_output_fig.line(
                x="timestamp",
                y=p,
                legend_label=f"Selected {p}",
                source=selected_path_material_output_source,
                color=palette[3 + i * 4],
            )
            material_output_fig.line(
                x="timestamp", y=p, legend_label=f"Reference {p}", source=reference_path_material_output_source, color=palette[4 + i * 4], alpha=0.8
            )
            material_output_fig.line(
                x="timestamp", y=p, legend_label=f"Ideal {p}", source=ideal_path_material_output_source, color=palette[5 + i * 4], alpha=0.8, line_dash="dotted"
            )
        ...
        doc.add_root(
            gridplot(
                [[scatter_fig], [path_fig], [material_input_fig, material_output_fig], [material_input_fig_volume, material_output_fig_volume]],
        ...
        def update() -> None:
            try:
                ...
                if best_solution:
                    ...
                    best_source.data = {
                        "f1": [best_solution.objectives[0]],
                        "f2": [best_solution.objectives[1]],
                    }
                ...
                if reference:
                    ...
                    reference_source.data = {
                        "f1": [reference.objectives[0]],
                        "f2": [reference.objectives[1]],
                    }
